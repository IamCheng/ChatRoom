...
{
	UpdateData(TRUE);
	server.init("127.0.0.1", _ttoi(m_port));
	unsigned int m_uThreadID;
	HANDLE 	m_hThreadHandle=(HANDLE)::_beginthreadex(NULL,0,ThreadFunction, &server, 0, &m_uThreadID);
	// TODO: 在此添加控件通知处理程序代码
}

unsigned __stdcall CChatRoomServerDlg:: ThreadFunction(LPVOID pThreadData)
{
	Server * tmp = (Server *) pThreadData;
	tmp->run();
	return 1;
}